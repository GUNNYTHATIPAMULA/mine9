...
const features = [
  ...
  },
  {
    ...
    description: '20 MNX per session',
  },
  {
    icon: Shield,
    title: 'Secure',
    description: 'Protected earnings',
  },
  {
    icon: Award,
    title: 'Referral Bonus',
    description: '+5% per invite',
  },
];
      ...
      <div className="flex-1 px-4 py-4 space-y-4">
        ...
        <div className="text-center animate-fade-in">
          ...
          <p className="text-sm text-muted-foreground">
            Earn 20 MNX per session {totalBoost > 0 && <span className="text-green-500">(+{totalBoost}% boost)</span>}
          </p>
        </div>

        {/* Mining Timer - Full Width */}
        ...
        <div className="grid grid-cols-2 gap-3 animate-fade-in" style={{ animationDelay: '0.2s' }}>
          {features.map((feature, idx) => (
            ...
            >
              ...
              <h3 className="font-display text-sm font-bold mb-0.5">{feature.title}</h3>
        ...
        <div className="glass-card p-4 animate-fade-in" style={{ animationDelay: '0.3s' }}>
          <h3 className="font-display text-lg font-bold mb-3">Mining Levels</h3>
          <div className="space-y-2">
            {miningTiers.map((tier, idx) => (
              <div
                key={idx}
                className={`flex items-center justify-between p-3 rounded-lg ${
                  tier.active ? 'bg-primary/20 border border-primary/50' : 'bg-secondary/30'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2.5 h-2.5 rounded-full ${
                      tier.active ? 'bg-primary animate-pulse' : 'bg-muted-foreground/30'
                    }`}
                  />
                  <div>
                    <p className="font-display text-sm font-bold">{tier.level}</p>
                    <p className="text-xs text-muted-foreground">{tier.range} MNX</p>
                  </div>
                </div>
                <span className="font-display text-sm font-bold gradient-gold-text">{tier.bonus}</span>
              </div>
            ))}
          </div>
        </div>
